...
interface PomodoroState {
  ...
  // API Actions
  saveFocusTime: (durationSeconds: number, taskId?: number | null) => Promise<void>;
...
export const usePomodoroStore = create<PomodoroState>()(
  persist(
    (set, get) => ({
      ...
      setPhase: (phase) => {
        const { settings, totalFocusTime, currentTaskId, phase: prevPhase } = get();
        ...
        set({ phase, timeLeft: duration * 60, isActive: false, totalFocusTime: 0 });
      },

      startTimer: () => {set({ isActive: true });console.log('Timer started')},
      ...
      tick: () => {
        const { timeLeft, isActive, settings, phase, totalFocusTime, focusSessionCount } = get();
        if (!isActive) return;
      ...
      setFocusTask: (taskId) => {
        ...
        if (phase === 'focus' && totalFocusTime > 0 && currentTaskId !== taskId) {
          get().saveFocusTime(totalFocusTime, currentTaskId);
          set({ totalFocusTime: 0 });
        }
      ...
      fetchDailySummary: async () => {
        try {
          const today = format(new Date(), 'yyyy-MM-dd');
          const response = await apiClient.get<DailySummary>('/api/focus-sessions/daily', {
            params: { date: today },
          });
      ...
      fetchTotalSummary: async () => {
        try {
          const response = await apiClient.get<TotalSummary>('/api/focus-sessions/total');
